...
|--------|-----------------------------|-----------------------|--------------------------|------------|
| A | **em uso — MENU e BUSCA** (26/07, fim de tarde): aba "Buscar" virou **"Catálogo"**, lupa subiu para o topo da Início abrindo a busca sobreposta (ROTEIRO 4.37). Antes: os COMENTÁRIOS (26/07, tarde): a nota de quem comenta voltou a aparecer no comentário, formato Amazon (ROTEIRO 4.5, atualização). ✅ **`AudioPlayer.tsx` liberado para a B** — terminei e commitei (modo carro em 1 toque, sem os diálogos de Bluetooth, ROTEIRO 4.33); `PersonProfile.tsx` também está livre ("Nota da obra" → "Nota da escrita", 4.15). Aba `🟢 JANELA A` | `client/src/components/layout/BottomNav.tsx`, `client/src/components/layout/TopNav.tsx`, `client/src/pages/Search.tsx`, `client/src/components/BuscaSobreposta.tsx` (novo) | não — serviço launchd sempre no ar | 26/07 |
| B | **em uso — o "Histórico de escuta" dos 3 pontinhos do PLAYER**. ⚠️ **O Matheus disse (26/07, tarde) que a A já terminou no `AudioPlayer.tsx` e me liberou o arquivo** — ele estava limpo no git quando assumi. Janela A: se ainda precisar dele, me avisa aqui. Feito antes hoje: Descobrir fundida na Buscar (4.32), proteção do áudio (4.34), aberturas adiadas (4.35). Aba `🔵 JANELA B` | `client/src/pages/AudioPlayer.tsx`, `client/src/lib/listening.ts`, componente novo do histórico | não — serviço launchd sempre no ar | 26/07 |
