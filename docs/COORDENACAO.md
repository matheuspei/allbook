...
|---|---|---|---|---|
| A · voz `pm_alex` · aba 🟢 | ✅ **31/08 — AS EDITORAS DE VERDADE (§4.148).** A ficha do livro mostra “Publicado por” com link para `/publisher/:slug`; o perfil lista o catálogo da casa, quem ela publica e quem narra para ela. 🚨 **B/C: a editora NÃO está no `catalogo.sqlite`** (só a Audible tem lá) — está no **`_ficha.json`, em `ficha.EDITORA`**, nas quatro lojas. **12.766 de 13.917 livros (91,7%)**, em **692 casas**. 🚨 **Slug de editora NUNCA muda** — é endereço e é por ele que o app guarda quem a pessoa acompanha; `resolverEditoras` (`script/editoras.ts`) reusa sempre o do banco. ⚠️ **`lib/publishers.ts` DERIVA do catálogo** (como `people.ts`): `Book.publisher` é o slug, o nome sai de `publisherNames` em `books.ts`. ⚠️ **`/api/catalogo` ganhou `editoras` e `publisher` por livro.** ⚠️ **Serviço novo `com.allbook.fichas`** (5h30, `scripts/sincronizar-fichas.sh`) traz a editora que o baixalivro corrigir; `npm run acervo fichas` força. 🚨 **A/B/C — dois defeitos de DADO que ninguém consertou ainda:** **176 dos 3.176 autores estão como “Poe, Edgar Allan”**, e ao menos 6 são o MESMO autor duplicado (“de Assis, Machado” + “Machado de Assis”); e **nome de editora entra como autor** (“Livro Falante”, “Charles River Editors”). ⚠️ Juntar perfis de pessoa NÃO é renomear: `/person/<slug>` já é endereço em uso, com foto e seguimento — merece passada própria, começando pelo baixalivro. | — livre — | não — serviço launchd sempre no ar | 31/08 |
| B · voz `pm_santa` · aba 🔵 | ✅ **31/08 — AUDIBLE FORA DA VITRINE (§4.145) · CAPAS UNIFORMES (§4.146) · SELETOR DE NARRAÇÃO (§4.147) · AGRUPAMENTO E PROGRESSO (§4.150)**. 🚨 **A: o autor sujo é seu terreno agora.** O Matheus achou o `storytel/11835367` (id 102924): *O pequeno príncipe — na voz de Glycon Luiz*, com **Glycon Luiz no autor, no narrador E na editora**. Eu consertei o AGRUPAMENTO (ele já acha as outras 5 vozes), mas a ficha continua dizendo "Escrito por Glycon Luiz" — isso é `lib/books.ts` + `BookDetails.tsx` + importação, que são seus. **Números: 768 livros com autor = editora** (o caso do "Editora Online" como autor do Pequeno Príncipe, id 108024) e 5.014 sem autor. ⚠️ **Autor = narrador (1.916) NÃO é sinal de erro** — autopublicado narra o próprio livro; tratá-los como sem autor funde livros homônimos. ⚠️ **`tituloDeVitrine()` corta no primeiro separador da LISTA, não no que aparece primeiro no texto** — em "O pequeno príncipe - Áudiolivro: Na voz de…" ele corta no lugar errado. Na vitrine é só estético; se alguém usar essa função para agrupar, quebra. 🚨 **DECISÃO DELE: o progresso é por NARRAÇÃO, não por obra** — durações diferentes, metade de uma não é metade da outra; não procurar conversão. | `client/src/lib/narrations.ts` · `client/src/components/SeletorDeNarracao.tsx` · `client/src/components/BookGrid.tsx` · `client/src/pages/Search.tsx` · `client/src/pages/RecommendationsEdit.tsx` · `client/src/pages/Comunidades.tsx` · `docs/ROTEIRO.md` · `CLAUDE.md` | não — serviço launchd sempre no ar | 31/08 |
