...
|--------|-----------------------------|-----------------------|--------------------------|------------|
| A | **em uso — PLAYER e MINI PLAYER** (27/07): o que acontece ao sair do player pela ficha do livro — a barrinha voltava mostrando "tocar" mesmo com o áudio rodando. Antes (26/07): aba "Buscar" → **"Catálogo"** e lupa na Início (4.37); nota de quem comenta de volta no comentário (4.5); modo carro em 1 toque (4.33); "Nota da obra" → "Nota da escrita" (4.15) — tudo commitado. Aba `🟢 JANELA A` | `client/src/components/layout/MiniPlayer.tsx`, `client/src/lib/playback.ts`, `client/src/pages/AudioPlayer.tsx` | não — serviço launchd sempre no ar (escutando na **rede local**; `HOST=127.0.0.1` fecha) | 27/07 |
