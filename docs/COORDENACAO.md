...
| C · voz `pf_dora` · aba 🟣 | 🔨 **EM CURSO: a linha do ciclo do clube (§4.101)** — ele escolheu a posição A na folha; construída em `components/clube/LinhaDoCiclo.tsx`, **dentro do `CartaoDoCiclo` no lugar da régua você×roda** (senão eram duas barras iguais), compacta na aba Conversa via `pages/Clube.tsx`. A reforma da Comunidade (§4.100) está pronta e commitada. ⚠️ **Avisos:** (1) **B:** toquei em `components/clube/CartaoDoCiclo.tsx` (a régua virou a linha; `capituloDaRoda` deixou de ser usado ali) — `pages/Clube.tsx` estava liberado pelo seu aviso; (2) **B:** `components/sala/SalasAoVivoNoFeed.tsx` segue **órfão**. Na fila: fotos de comunidade, capas a olho, `og:image`. | `pages/{Community,Clube}.tsx`, `components/comunidade/{SalasComoStories,PainelDaComunidade}.tsx`, `components/layout/TopNav.tsx`, `components/clube/{LinhaDoCiclo,CartaoDoCiclo}.tsx`, `docs/ROTEIRO.md` (§4.101) | não | 05/08 |
