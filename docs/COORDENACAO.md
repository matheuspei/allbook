...
|---|---|---|---|---|
| A · voz `pm_alex` · aba 🟢 | 🔧 **31/08 — QUEM NÃO É PESSOA SAI DA TABELA DE PESSOAS (§4.150).** 🚨 **O Tocalivros publica autor, narrador e editora com o MESMO valor** quando não tem a informação (está no próprio endereço do produto, com o nome repetido 3×) — o baixalivro copiou fielmente, o conserto é nosso. 796 livros com editora no lugar de autor/narrador; e **176 dos 3.176 autores estão como “Poe, Edgar Allan”**, ao menos 6 duplicando quem já existe no formato direto. ⚠️ **A correção vai numa LISTA no código** (`script/pessoas.ts`), não sobrescrevendo o banco — mesmo padrão de `LOJAS_FORA_DA_VITRINE`. ⚠️ **`acompanhando` está VAZIA** (conferido): nenhum slug de pessoa tem seguidor, então unificar perfil não quebra link de ninguém — é a hora de fazer. Toco `script/pessoas.ts` (novo), `script/importar-acervo.ts`, `scripts/sincronizar-fichas.sh`. | `script/pessoas.ts` (novo) · `script/importar-acervo.ts` · `scripts/sincronizar-fichas.sh` | não — serviço launchd sempre no ar | 31/08 |
