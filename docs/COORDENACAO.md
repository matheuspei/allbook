...
| C | **assumida — 31/07: AUDITORIA DA COMUNIDADE** (pedido do Matheus: *"tem muita coisa que era pra ser tirada e não foi, e muita que era pra ser colocada e não foi"*). Conferi §4.53–§4.58 contra o código e registrei o resultado no **ROTEIRO §4.72**. Por ora só toquei em `docs/ROTEIRO.md` e neste arquivo — **zero arquivo do app**. Quando for construir os buracos (menu "…" da Comunidade, responder pergunta com livro + melhor resposta, notificação de curtida agrupada), declaro os arquivos aqui antes. Aba `🟣 JANELA C` | `docs/ROTEIRO.md` | não — serviço launchd sempre no ar | 31/07 |
