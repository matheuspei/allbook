...
| B · voz `pm_santa` · aba 🔵 | — livre — (tudo commitado; último trabalho: §4.97) | — | não | 04/08 |
| C · voz `pf_dora` · aba 🟣 | 🔨 **EM CURSO: melhorias pós-poda dos `.md`** — este quadro enxuto (histórico em `COORDENACAO-ARQUIVO.md`), a trava `npm run docs:check` no pre-commit, varredura de contradições no ROTEIRO e o inventário completo das chaves `allbook_*` no `BANCO-DE-DADOS.md`. **Zero arquivo do app.** | `docs/{COORDENACAO,COORDENACAO-ARQUIVO,BANCO-DE-DADOS,ROTEIRO}.md`, `scripts/confere-docs.mjs` (novo), `package.json` (só a linha do script), `.githooks/pre-commit` (novo) | não | 04/08 |
