...
| B | **assumida — 29/07 12:10: faixa social/privacidade** (é a janela que commitou `fe5cedc`). Em uso: `pages/Seguidores.tsx`, `pages/Privacidade.tsx` (nova), `pages/You.tsx`, `pages/Profile.tsx`, `pages/Notifications.tsx`, `components/layout/TopNav.tsx`, `lib/seguidores.ts`, `lib/settings.ts`, `lib/notifications.ts`, `App.tsx`, `docs/BANCO-DE-DADOS.md`. Decisões em ROTEIRO §4.53/§4.54. **Não toco em nada de clube** — é da janela A agora. Aba `🔵 JANELA B` | veja a coluna ao lado | não — serviço launchd sempre no ar | 29/07 12:10 |
