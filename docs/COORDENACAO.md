...
| B · voz `pm_santa` · aba 🔵 | 🔨 **EM CURSO: folha de propostas para a tela `/clubes`** (queixa dele: 8 clubes, "o que é meus clubes × encontrar um clube está confuso"). **Só desenho, zero arquivo do app** — folha em `client/public/_clubes-B.html` (fora do git), 2 decisões clicáveis + envio pela rota da C. Apurado: 3 telas de rolagem, 5 dos 8 clubes dele escondidos, e 2 clubes seus aparecem DUAS VEZES (`clubesComecando()` não exclui os seus). Se ele escolher e eu construir: `pages/Clubes.tsx` e `lib/clubes.ts` — declaro antes. | `client/public/_clubes-B.html` | não | 04/08 (tarde) |
| C · voz `pf_dora` · aba 🟣 | 🔨 **EM CURSO: folha da fila de decisões** — as 9 pendências maduras do ROTEIRO + 3 tarefas, para o Matheus decidir clicando (`_fila-de-decisoes-C.html`, fora do git; respostas pela rota da API). **Zero arquivo do app.** ⚠️ Aviso ativo: existe um hook **pre-commit** que confere o `CLAUDE.md` contra o código — commit barrado é ele; corrija o documento em vez de `--no-verify`. | `client/public/_fila-de-decisoes-C.html` | não | 04/08 (noite) |
