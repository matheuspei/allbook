...
|---|---|---|---|---|
| A · voz `pm_alex` · aba 🟢 | — livre — (última tarefa: o painel do hambúrguer virou o **Menu global**, §4.103 — o botão está em toda tela com TopNav; "Minhas notas"/"Meus trechos" migraram do `/you` para lá). ⚠️ Avisos: (1) B/C: o título da gaveta agora é "Menu" e a §4.57 foi revogada nesse ponto; (2) C: o seu aviso do TopNav atrás do voltar da ficha segue de pé — não toquei nisso. | — | não — serviço launchd sempre no ar | 05/08 |
...
| C · voz `pf_dora` · aba 🟣 | ⏳ **AGUARDANDO: a folha da averiguação de design** (`/_averiguacao-de-design-C.html`, 05/08) — 6 decisões (resumo em inglês, cabeçalhos da ficha, vazamento nas barras, sala sem fim, capas repetidas, herói da Início) + limpezas rápidas; ele responde pela folha (`_respostas-de-folhas/averiguacao-de-design-C.json`). ⚠️ Avisos: (1) **A:** dois achados na sua faixa — o resumo da ficha vem **em inglês** do `catalog-enriched.ts` (synopsis sem tradução), e o TopNav fica visível atrás do botão de voltar da ficha; a decisão dele sai na folha; (2) `components/sala/SalasAoVivoNoFeed.tsx` segue **órfão**. | folha `client/public/_averiguacao-de-design-C.html` (fora do git) | não | 05/08 |
