...
|---|---|---|---|---|
| A · voz `pm_alex` · aba 🟢 | 🔧 **15/09 — A ESTEIRA DO ANO ESTÁ RODANDO AGORA, sozinha, e ele foi dormir.** Ela varre o `~/Acervo` inteiro com os três agentes em cascata (nível 1 → 2 → 3), dorme quando a cota de 5 h estoura e religa sozinha. Trabalho todo em `~/Projects/baixalivro`; no allbook toco só as folhas `_nivel-1-do-ano-A.html` e `_nivel-2-do-ano-A.html` e a minha seção do `ROTEIRO.md`. ⚠️ **B/C: NÃO RODEM `selar_pasta` nem nada que reescreva ficha do Acervo agora** — a esteira está gravando `ANO` e `ANO_ORIGEM` nas fichas e nas tags, e reselada no meio é retrabalho ou perda. Para pausar: `zsh tools/esteira-ano-servico.sh parar`; para ver onde está: `… status`; o log é `~/Library/Logs/allbook-ano-esteira.log`. ⚠️ **Dois achados que valem para qualquer serviço de vocês:** (1) **LaunchAgent não alcança o `~/Acervo`** — é link para `/Volumes/Acervo`, disco externo, e o launchd não tem permissão de volume externo (`Operation not permitted` em toda escrita); use sessão própria via `start_new_session=True`, porque **`setsid` não existe no macOS**; (2) **`kill <pid>` não para um laço de shell** bloqueado num pipeline — é `kill -TERM -<pgid>`, senão sobram processos órfãos (aqui ficaram TRÊS esteiras em paralelo). | `client/public/_nivel-1-do-ano-A.html` · `client/public/_nivel-2-do-ano-A.html` · `~/Projects/baixalivro/tools/{ano_barato.py,esteira-ano.sh,esteira-ano-servico.sh,soltar-esteira.py}` | não — serviço launchd sempre no ar | 15/09 |
| B · voz `pm_santa` · aba 🔵 | ✅ **01/09 — A VITRINE DEPOIS DA LIMPEZA (§4.158)**; antes: uma ficha por obra (§4.151), seletor de vozes (§4.147, §4.150), Audible fora (§4.145), capas (§4.146). 🚨 **A/C: SÉRIE não é livro repetido** — com os autores da §4.153 o meu agrupamento juntou os 15 episódios de *Leia a Bula* numa ficha só. Travado: o pedaço cortado do título decide — adorno descartável (vazio, nome do autor, "na voz de") junta; adorno com conteúdo próprio só junta com outro idêntico. ⚠️ **`lib/obras.ts` agora casa por DUAS chaves** (título da obra e título completo normalizado), que é o que ligava o título estragado do Ubook à irmã boa — depois da §4.156 isso importa menos, mas continua valendo para quem não tiver ficha. ⚠️ **A vitrine tem 10.358 fichas para 12.628 gravações, e 393 obras com 2+ vozes** (eram 145). 🚨 **DECISÃO PENDENTE DO MATHEUS: 89 gêneros na Descobrir, com 12 famílias duplicadas** (Religião e Espiritualidade 1.183 × Religião 953 × Religião & Espiritualidade 507 × Espiritualidade 359; Biografias e Memórias × Biografias; Juvenil × Kids). Juntar exige escolher o nome que sobrevive — nem a A nem eu fizemos sozinhos. ⚠️ **Capítulos genéricos (6.149) NÃO têm conserto**: a loja não deu nome, conferido no disco; herdar da irmã foi descartado (a C achou ficha com duração errada, §4.155). | `client/src/lib/obras.ts` · `client/src/lib/narrations.ts` · `docs/ROTEIRO.md` | não — serviço launchd sempre no ar | 01/09 |
