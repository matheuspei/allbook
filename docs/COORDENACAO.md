...
|---|---|---|---|---|
| A · voz `pm_alex` · aba 🟢 | 🔧 **21/09 — OUVIR EM CASA, e os capítulos fora de ordem (§4.164).** 🚨 **B: eu toquei `server/audio.ts` e `server/seguranca.ts`** — o player que você consertou agora toca **sem conta** quando o pedido vem de `localhost` ou da rede privada (`ehDaCasa`), porque ele reclamou do login que não pediu; pelo túnel continua 401. `emProducao` virou `export` em `seguranca.ts`. ⚠️ **E achei um defeito de dado que pode afetar o seu teste: a importação ordenou os capítulos ALFABETICAMENTE** — `"10" < "100" < "11"` —, e todo livro com 100+ capítulos estava com a ordem trocada desde o começo (93 dos 103 do *Touchdown*). Reordenei 12.824 capítulos em 59 livros; os que tocam inteiros foram de 13.854 para **13.908 de 13.917**. 🚨 **A CAUSA CONTINUA LÁ**: `script/importar-acervo.ts` ordena por string; quem mexer nele, ordene pelo número extraído do nome. ⚠️ **O ano da obra está em 2.680 e a cascata de agentes pagos está DESLIGADA** (§4.159) — 14.480 livros dariam US$ 12 mil. Dois testes acham ano errado de graça: `auditar_anos.py divergencia` e `outlier`. | `server/audio.ts` · `server/seguranca.ts` · `~/Projects/baixalivro/tools/{ano_de_graca.py,auditar_anos.py,ano_wikidata_autor.py}` | não — serviço launchd sempre no ar | 21/09 |
