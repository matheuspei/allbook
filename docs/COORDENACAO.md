...
|---|---|---|---|---|
| A · voz `pm_alex` · aba 🟢 | — livre — (última tarefa: §4.103, 2ª rodada — o Menu absorveu o `/you`). ⚠️ **B/C, dois recados que mudam texto de tela:** (1) a aba de baixo agora se chama **"Novidades"**, não "Comunidade" — a palavra *comunidade* vale **só para os fóruns** (`/forum`), porque o mesmo nome levava a duas telas; ao escrever texto que cite a aba, é "Novidades"; (2) `/you` virou **"Configurações"** e só tem ajustes — notas, trechos, downloads, estatísticas e conquistas moram no **Menu** do hambúrguer. Some também o cartão vermelho de sala ao vivo e a bolinha do hambúrguer. | — | não — serviço launchd sempre no ar | 05/08 |
| B · voz `pm_santa` · aba 🔵 | — livre — (última tarefa: os ajustes do perfil viraram **uma lista só**, §4.105). ⚠️ **A, dois recados:** (1) **há 4 linhas minhas dentro do seu `You.tsx` não commitado** — a linha do painel virou `label: "Perfil e privacidade"`, com comentário citando a §4.105; deixei o arquivo **fora do meu commit** porque o diff está com as suas 89 linhas dentro, então ele sobe no seu; (2) a tela `/privacidade` agora se chama **"Perfil e privacidade"** e mostra a lista inteira (foto/bio, o que a página mostra, conta privada, sino) — a rota não mudou. ⚠️ Avisos da tarefa anterior: `Grupo.tsx`/`Topico.tsx` NÃO mudaram (estrutura Orkut RECONFIRMADA em 05/08) e `lib/grupos.ts` ganhou 14 comunidades semeadas (16 → 30). | — | não | 05/08 |
| C · voz `pm_alex` HOJE (a `pf_dora` foi tomada por outra sessão às 14:53 — aviso dado a ele por voz) · aba 🟣 | 🔨 **EM CURSO: o cabeçalho do topo se esconde ao rolar para baixo e volta ao rolar para cima** (pedido dele de 05/08, à la YouTube/Instagram; nasceu do elogio à ficha sem TopNav). Os `sticky top-14` (PageHeader, chips da Biblioteca) sobem junto para `top-0`. ⚠️ **A:** vou editar o `TopNav.tsx` (só as classes do `<header>` + um hook novo) — NÃO toco em `PainelDaComunidade.tsx` nem no `BottomNav.tsx`. ⚠️ **B:** o `pages/Profile.tsx` (seu) tem um `sticky top-14` na linha ~223 que vai ficar com vão de 56px quando o topo se esconder — quando liberar, aplico lá também (ou aplique você: hook `use-cabecalho-recolhido`). Avisos antigos que seguem valendo: livro novo no `books.ts` pede sinopse em `lib/sinopses.ts`; `components/sala/SalasAoVivoNoFeed.tsx` órfão. | `components/layout/TopNav.tsx`, `components/PageHeader.tsx`, `pages/Library.tsx`, `hooks/use-cabecalho-recolhido.ts` (novo) | não | 05/08 |
