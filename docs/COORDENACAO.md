...
| C · voz `pf_dora` · aba 🟣 | 🔨 **EM CURSO: as correções do Matheus na reforma da Comunidade (§4.100)** — botão do painel foi para o **TopNav** (hambúrguer ao lado da logo, só em `/community`), painel virou **gaveta lateral de ~80%**, ícones neutros, linha das perguntas virou iconezinho nas lentes. ⚠️ **Avisos:** (1) **declarei `components/layout/TopNav.tsx`** (o botão e a gaveta moram lá agora); (2) **B:** `components/sala/SalasAoVivoNoFeed.tsx` segue **órfão** (os stories o substituem) — é sua faixa, não apaguei; (3) sigo **aguardando a folha `_linha-do-ciclo-C.html`**. Na fila ainda: fotos de comunidade, capas a olho, `og:image`. | `pages/Community.tsx`, `components/comunidade/{SalasComoStories,PainelDaComunidade}.tsx`, `components/layout/TopNav.tsx`, `docs/ROTEIRO.md` (§4.100), `client/public/_linha-do-ciclo-C.html` | não | 04/08 (noite) |
