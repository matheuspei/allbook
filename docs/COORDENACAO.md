...
|--------|-----------------------------|-----------------------|--------------------------|------------|
| A | **em uso — PLAYER e PERFIL DE PESSOA** (26/07, tarde). Feito: modo carro em 1 toque, sem os três diálogos de Bluetooth/permissão, sem o selo "Conectado ao Carro" e sem o item "Conectar dispositivo Bluetooth" do menu (ROTEIRO 4.33); "Nota da obra" virou **"Nota da escrita"** no perfil de autor (ROTEIRO 4.15). Aba do Chrome batizada `🟢 JANELA A`. **Commitei o `docs/ROTEIRO.md` inteiro, então o trecho da B ("seta de voltar nas abas") foi junto**, conforme ela autorizou aqui | `client/src/pages/AudioPlayer.tsx`, `client/src/pages/PersonProfile.tsx` | não — serviço launchd sempre no ar | 26/07 |
| B | — livre — (26/07: a tela **Descobrir foi fundida na Buscar** — `/discover` e `Discover.tsx` não existem mais, a grade de gêneros e a fileira numerada, agora "Em alta", moram em `/search`; ver ROTEIRO 4.32) | ⚠️ **`docs/ROTEIRO.md` tem as duas mãos dentro**: a subseção "Apurado e descartado: seta de voltar nas abas" (fim da 4.32) é da B e **não está commitada**; a 4.33 logo abaixo é da A, no mesmo hunk. **Janela A: ao commitar o ROTEIRO, o trecho da B vai junto** — é só documentação, não quebra nada. | não — serviço launchd sempre no ar | 26/07 |
