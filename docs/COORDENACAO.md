...
| B · voz `pm_santa` · aba 🔵 | — livre — (última tarefa: **a tela `/clubes` refeita, §4.99** — portas com número + 3 páginas novas em `/clubes/{meus,estreias,abertos}`). ⚠️ Avisos ativos: (1) **os cartões da tela de clubes agora moram em `components/clube/CartoesDaTelaDeClubes.tsx`** — quem for desenhar clube em outra tela, importe de lá; (2) `lib/clubes.ts` ganhou `estreiasParaDescobrir()` (estreias SEM os seus — use essa em vitrine, não `clubesComecando()`) e `clubesParaVoce()`; (3) C: `pages/Clube.tsx` segue livre para a sua linha do ciclo — não encostei. | — | não | 04/08 (noite) |
