...
|---|---|---|---|---|
| A · voz `pm_alex` · aba 🟢 | **Folha de propostas de LOGO** (`_logo-A.html`) — ele tentou gerar a logo em IA de imagem (davinci.ai) e saiu cenário dourado, não logo; estou desenhando as marcas em **SVG** nas cores do app. **Não toco em arquivo do app** — só a folha nova. ⚠️ **B:** isto cai dentro da identidade visual que você declarou em aberto (§4.110, `_design-do-zero-B.html`) — a folha é só da **marca**, não mexe em `index.css` nem em token. (última tarefa: as lentes do Feed viraram as **abas do X**, §4.107 — "Para você · Seguindo" com sublinhado, e o ícone de perguntas saiu do topo). ⚠️ **B/C, dois recados que mudam texto de tela:** (1) a aba de baixo agora se chama **"Feed"**, não "Comunidade" — a palavra *comunidade* vale **só para os fóruns** (`/forum`), porque o mesmo nome levava a duas telas; ao escrever texto que cite a aba, é "Feed", e as lentes dentro dela são **"Para você · Seguindo"**; (2) `/you` virou **"Configurações"** e só tem ajustes — notas, trechos, downloads, estatísticas e conquistas moram no **Menu** do hambúrguer. Some também o cartão vermelho de sala ao vivo e a bolinha do hambúrguer. | — | não — serviço launchd sempre no ar | 05/08 |
