...
|---|---|---|---|---|
| A · voz `pm_alex` · aba 🟢 | 🔧 **01/09 — O AUTOR QUE FALTA (§4.152).** 🚨 **A proposta de APAGAR o nome da editora do campo do autor foi derrubada pelo Matheus** — ele achou o autor no site da editora em um clique. A regra agora é: **antes de apagar dado ruim, procurar o bom.** 🚨 **36% do acervo não tem autor**: 5.014 livros dizem “Autor desconhecido” (4.938 do Ubook) e 5.016 “Narrador não informado” — muito maior que os 819 com editora no lugar do autor. ⚠️ **B/C: a melhor fonte de autor é o próprio acervo** — gravação irmã da mesma obra; 1.242 livros se resolvem assim, e 92 deles hoje são **cartão repetido na vitrine** (o *Pequeno Príncipe* da Editora Online não entra no grupo porque tem “Editora Online” no campo do autor). ⚠️ **A herança tem trava**: título curto e genérico não herda (senão *Inteligência emocional* ganharia Gilclér Regina). ⚠️ **A correção vai num arquivo do código com a FONTE junto**, aplicada por leitura — não sobrescreve o banco. Folha aberta: `_autor-que-falta-A.html`. Toco `script/pessoas.ts` (novo), `script/importar-acervo.ts`, `scripts/sincronizar-fichas.sh`. | `script/pessoas.ts` (novo) · `script/importar-acervo.ts` · `scripts/sincronizar-fichas.sh` | não — serviço launchd sempre no ar | 01/09 |
