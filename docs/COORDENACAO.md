...
| B · voz `pm_santa` · aba 🔵 | 🔨 **EM CURSO: CONSTRUINDO a tela `/clubes` escolhida (M3 da folha `_clubes-B2.html`)** — ele decidiu por voz: portas com número no estilo do perfil (**Meus 8 · Estreias · Abertos**), cada uma abrindo página própria; + as regras da 1ª folha: busca no topo, estreia sua com etiqueta entre os seus (o carrossel/página de estreias **exclui os seus** — mata a duplicata), "Para você" com motivo. ⚠️ **Declaro:** `pages/Clubes.tsx` (reescrever), `pages/ListasDeClubes.tsx` (**novo** — /clubes/meus, /clubes/estreias, /clubes/abertos), `components/clube/CartoesDaTelaDeClubes.tsx` (**novo** — cartões extraídos de Clubes.tsx), `lib/clubes.ts` (funções novas, nada removido), `App.tsx` (3 rotas). ⚠️ A: não encosto na sua `EncontrosDoClube` nem em `pages/Clube.tsx`. | `pages/{Clubes,ListasDeClubes}.tsx`, `components/clube/CartoesDaTelaDeClubes.tsx`, `lib/clubes.ts`, `App.tsx` | não | 04/08 (noite) |
| C · voz `pf_dora` · aba 🟣 | ⏸️ **Aguardando o Matheus na folha `_linha-do-ciclo-C.html`** (posição da linha do ciclo do clube — se ele escolher, construo e aí encosto em `pages/Clube.tsx`/`components/clube/*`, ⚠️ **combinando antes com a B**, que está na faixa de clubes). As 3 construções da folha da fila estão commitadas (`3cc424c`); **na minha fila, aprovadas por ele (§4.98): as 10 fotos de comunidade, a conferência de capas a olho e a `og:image`**. ⚠️ Aviso ativo: hook **pre-commit** confere o `CLAUDE.md` contra o código — commit barrado é ele. | `client/public/_linha-do-ciclo-C.html` | não | 04/08 (noite) |
