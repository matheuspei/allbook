...
| B · voz `pm_santa` · aba 🔵 | ✅ **31/08 — (1) A AUDIBLE FORA DA VITRINE (§4.141)** e **(2) AS CAPAS UNIFORMES (§4.142)**. 🚨 **Nada da Audible foi apagado**: os 1.289 livros seguem no banco; quem os esconde é `LOJAS_FORA_DA_VITRINE` em `server/catalogo.ts`, só na leitura (tirar `audible` da lista + reiniciar traz de volta). A vitrine tem 12.628 livros e 21 gêneros — a lista de gêneros passou a devolver só gênero com livro visível. 🚨 **A/C: `line-clamp-*` DEFINE o `display`** — `block` ao lado dele desliga o corte em silêncio, e era isso que fazia um título de 10 linhas esticar a grade de capas. Varridos os 6 lugares do app. E **cartão de livro que é `<button>` precisa de `flex flex-col`**: botão esticado centraliza o próprio conteúdo, e a capa do vizinho desce. ⚠️ **C: `npm run check` está com 4 erros de `durationSec` possivelmente nulo** (`salaAoVivo.ts`, `AudioPlayer.tsx`, `BookDetails.tsx`) — não são meus, apareceram durante o meu trabalho. ⚠️ **C: o §4.141 do CLAUDE.md (capítulos reais) colide com o §4.141 do ROTEIRO (Audible)** — o próximo número livre é 4.143. | `server/catalogo.ts` · `client/src/components/BookGrid.tsx` · `client/src/pages/Search.tsx` · `client/src/pages/RecommendationsEdit.tsx` · `client/src/pages/Comunidades.tsx` · `docs/ROTEIRO.md` · `CLAUDE.md` | não — serviço launchd sempre no ar | 31/08 |
