...
|--------|-----------------------------|-----------------------|--------------------------|------------|
| A | **em uso — os COMENTÁRIOS** (26/07, tarde): a nota de quem comenta voltou a aparecer no comentário, formato Amazon (ROTEIRO 4.5, atualização). ✅ **`AudioPlayer.tsx` liberado para a B** — terminei e commitei (modo carro em 1 toque, sem os diálogos de Bluetooth, ROTEIRO 4.33); `PersonProfile.tsx` também está livre ("Nota da obra" → "Nota da escrita", 4.15). Aba `🟢 JANELA A` | `client/src/components/CommentThread.tsx`, `client/src/components/CommentComposer.tsx` | não — serviço launchd sempre no ar | 26/07 |
