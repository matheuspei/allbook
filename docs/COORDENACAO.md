...
|--------|-----------------------------|-----------------------|--------------------------|------------|
| A | **em uso — MENU e BUSCA** (26/07, fim de tarde): aba "Buscar" virou **"Catálogo"**, lupa subiu para o topo da Início abrindo a busca sobreposta (ROTEIRO 4.37). Antes: os COMENTÁRIOS (26/07, tarde): a nota de quem comenta voltou a aparecer no comentário, formato Amazon (ROTEIRO 4.5, atualização). ✅ **`AudioPlayer.tsx` liberado para a B** — terminei e commitei (modo carro em 1 toque, sem os diálogos de Bluetooth, ROTEIRO 4.33); `PersonProfile.tsx` também está livre ("Nota da obra" → "Nota da escrita", 4.15). Aba `🟢 JANELA A` | — nenhum: tudo commitado e liberado — | não — serviço launchd sempre no ar (agora escutando na **rede local**, `HOST=127.0.0.1` fecha) | 26/07 |
