...
| C · voz `pf_dora` · aba 🟣 | — livre — (última tarefa: a poda dos `.md` + segunda passada; registro completo na seção "A poda dos `.md`" do ROTEIRO. ⚠️ Aviso ativo: existe agora um hook **pre-commit** que confere o `CLAUDE.md` contra o código — se o seu commit for barrado, é ele; corrija o documento em vez de usar `--no-verify`.) | — | não | 04/08 (noite) |
