...
| B · voz `pm_santa` · aba 🔵 | 🔨 **EM CURSO: folhas de propostas para a tela `/clubes`** (queixa dele: 8 clubes, "meus × encontrar está confuso"). **Só desenho, zero arquivo do app.** Ele respondeu à 1ª folha (`_clubes-B.html`) por voz — gostos de A e B + regras novas: **busca sempre no topo**, separação abrindo **outra página**, estreia sua com etiqueta entre os seus, e "Para você" (clubes recomendados, à la portas do perfil). A 2ª folha (`_clubes-B2.html`) desenha 3 mesclas (porta/abas/portas-do-perfil), 2 telefones cada. Apurado antes: 3 telas de rolagem, 5 de 8 escondidos, 2 clubes seus DUPLICADOS (`clubesComecando()` não exclui os seus). Se ele escolher e eu construir: `pages/Clubes.tsx` e `lib/clubes.ts` — declaro antes. | `client/public/_clubes-B.html`, `client/public/_clubes-B2.html` | não | 04/08 (noite) |
| C · voz `pf_dora` · aba 🟣 | 🔨 **EM CURSO: construindo as decisões da folha da fila (§4.98)** — o Matheus respondeu; agora: tirar o **modo carro** do player, dividir **/perguntas** em sem-resposta/respondidas, e a fileira **"Ouvindo agora"** no topo da Comunidade (lente Todos). ⚠️ A: entro em `pages/AudioPlayer.tsx` e `components/BarraDeAcoesDoPlayer.tsx` (modo carro) e em `pages/Community.tsx` (só para **inserir** a fileira no topo — não toco na sua `LinhaDeAtividade`); conferindo `git diff` antes de cada `add`. ⚠️ Aviso ativo: hook **pre-commit** confere o `CLAUDE.md` contra o código — commit barrado é ele. | `pages/{AudioPlayer,Perguntas,Community}.tsx`, `components/BarraDeAcoesDoPlayer.tsx`, `components/comunidade/OuvindoAgora.tsx` (novo), `client/public/_fila-de-decisoes-C.html`, `docs/ROTEIRO.md` (§4.98) | não | 04/08 (noite) |
