...
|---|---|---|---|---|
| A · voz `pm_alex` · aba 🟢 | 🔧 **15/09 — QUEM NÃO É GENTE SAI DA TABELA DE PESSOAS.** ✅ **§4.159 no ar:** campo sem nome **não escreve “não informado”, a linha some** (decisão dele). Isso matou os perfis-fantasma de “Autor desconhecido” (70 livros) e “Narrador não informado” (234) — eram pessoas com avatar e botão de seguir. A régua é `ehGente()` em `lib/books.ts`, e **`autoresDe`/`narradoresDe` agora podem devolver lista vazia**. ⚠️ **B: renumerei a minha “Um livro em vários gêneros” de §4.157 para §4.158** — nós dois usamos 4.157 em 01/09. A sua ficou. 🔧 **Fazendo agora:** `script/pessoas.ts` — a lista de nomes que são organização ocupando campo de gente (**1.092 livros**: Max Editorial 178, Cresça Brasil 160, LIBROTEKA 93…) e a unificação dos **196 nomes invertidos** (“de Assis, Machado” → “Machado de Assis”), as duas aprovadas por ele. Aplicadas por leitura, sem sobrescrever o banco. | `script/pessoas.ts` (novo) · `script/importar-acervo.ts` · `client/src/lib/books.ts` · `client/src/pages/BookDetails.tsx` · `client/src/components/SeletorDeNarracao.tsx` | não — serviço launchd sempre no ar | 15/09 |
