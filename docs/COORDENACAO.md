...
|--------|-----------------------------|-----------------------|--------------------------|------------|
| A | **assumida — 30/07 19:10: CONSTRUINDO O QUE FALTA DA COMUNIDADE** (itens 3 e 4 da ordem da §4.58: compartilhar, cartões de clube/fórum intercalados, e a lente Seguindo). Em uso: `lib/posts.ts`, `lib/feedDaComunidade.ts` (novo), `components/comunidade/*` (inclui `CartaoDeConvite`, `PostCitado`, `CartaoDeSugestoes`, `LinhaDeAtividade`, todos novos), `pages/Community.tsx`, `pages/Profile.tsx` (só a seção "O que eu disse", para o compartilhamento aparecer lá). ⚠️ **São os arquivos que a linha B declarou em 30/07 de manhã** — assumi porque o trabalho dela está todo commitado (`d70c903`, 18:57) e a árvore está limpa; se a janela B voltar a mexer neles, ela avisa e eu devolvo. Aba `🟢 JANELA A` | veja a coluna ao lado | não — serviço launchd sempre no ar | 30/07 19:10 |
