...
| C · voz `pf_dora` · aba 🟣 | 🔧 **30/08 — OS CARIMBOS DA SINCRONIZAÇÃO (§4.138) + o título na vitrine (§4.137).** 🚨 **A/B: `livros` ganhou 4 COLUNAS e o `db:push` já rodou** — `subtitulo`, `ficha_gerada_em`, `vinheta_ficha`, `vinheta_ingerida` (só ADD COLUMN, nada apagado). ⚠️ **O importador PAROU de colar título e subtítulo** (`titulo` agora é só o título; `Book.subtitle` é campo novo) — quem escrever tela com título grande usa `tituloDeVitrine(book)` de `lib/books.ts` **e** `line-clamp`, porque 1.147 títulos longos não têm separador nenhum. ⚠️ **`npm run acervo` mudou de saída**: passa a dizer quantas fichas mudaram desde a última importação e quantos áudios foram ingeridos com vinheta velha. ⚠️ **`vinheta_ingerida` é escrita SÓ pelo `npm run audio`** — não a inclua num `set` de importação, ou o carimbo do áudio se perde. | `shared/schema/catalogo.ts` · `script/importar-acervo.ts` · `script/ingerir-audio.ts` · `script/carimbos.ts` (novo) · `server/catalogo.ts` · `client/src/lib/books.ts` · `client/src/pages/Home.tsx` · `client/src/pages/BookDetails.tsx` · `client/src/components/SearchResults.tsx` | não | 30/08 |
