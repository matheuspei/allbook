...
| C | **assumida — 31/07 (noite): FAXINA** (pedido do Matheus: código morto, arquivos mortos e testes desnecessários). Voz **`pf_dora`** (trocada a pedido dele; a reserva `pm_alex` voltou para a janela de PID 95091). Em uso: `docs/COORDENACAO.md` (esta linha) e as folhas mortas de `client/public/` (as `_*.html` de 28–29/07, **13 apagadas com cópia no scratchpad**; ficaram `_ponte-conversas.html` e `_pontes-prototipo.html` — proposta em aberto, §4.87 — e `_sala-ao-vivo-B.html`, declarada pela B). **Não toco em nenhum arquivo declarado por A ou B.** Aba `🟣 JANELA C` | veja a coluna ao lado | não — serviço launchd sempre no ar | 31/07 (noite) |
